...
option_list = list(
  ...
  make_option(c("-p", "--prefix"), action="store", default='', type='character', help="Where to put the output files")
)
opt = parse_args(OptionParser(option_list=option_list))

#check args are provided
if (opt$bamlist == "" || opt$bed == "" || opt$fasta == "" || opt$prefix == ""){
  print(script.help)
  stop()
}
...
for (i in 1:nsamples) {
  ...
  sampleName <- gsub("\\.bam", "", gsub("", "", originalFilename))
  ...
  #write header
  vcf.filename <- paste(c(prefix, outputPrefix, "_cnv.vcf"), collapse="")
  write(
    paste(
        "##fileformat=VCFv4.1\n",
  ...
  write.table(data.frame(all.exons@annotations$chromosome, all.exons@annotations$start, all.exons@annotations$end, all.exons@annotations$name, all.exons@reference, all.exons@test, all.exons@likelihood, stringsAsFactors=FALSE), sep="\t", row.names=FALSE, col.names=c("chr", "start", "end", "name", "ref", sampleName, "0.5", "1", "1.5"), quote=FALSE, file=paste(prefix, sampleName, "_cnv.txt", sep=""))
